...
| Concurrent appointment booking | Database invariant and race-condition testing | Two independent EF contexts attempt the same doctor and time concurrently. Application checks provide a friendly response, while SQL Server's unique index is the final authority that permits only one slot row. Testing with the real provider proves behavior an in-memory database cannot reproduce. | `Services/AvailabilityService.cs`, `Data/ApplicationDbContext.cs`, `AppointmentWorkflowTests.cs` |
| Appointment state transition | State consistency across related records | Approval changes the request and reserves its slot only after contact confirmation. Rejection clears approval metadata and releases the same slot so another patient can use it. A SQL-backed test verifies the records agree after each staff decision. | `Areas/Admin/Controllers/AppointmentRequestsController.cs`, `AppointmentWorkflowTests.cs` |
