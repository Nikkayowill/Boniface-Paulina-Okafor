...
| Hosting health signals | Liveness and readiness separation | The host can distinguish a running web process from an application that can actually reach PostgreSQL. This prevents traffic from being routed to an instance that is alive but unable to serve database workflows. | `Services/DatabaseHealthCheck.cs`, `Program.cs` |
...
| Development SQL readiness gate | Liveness versus dependency readiness | The verification script first proves PostgreSQL can answer a query, then applies migrations through the same controlled application path used by deployment. `/health/live` proves the web process runs, while `/health/ready` proves it can also reach PostgreSQL. | `scripts/verify-development-sql.sh`, `Services/DatabaseHealthCheck.cs`, `Program.cs` |
...
| Deterministic notification routing | Configuration normalization and safe defaults | Provider names are resolved case-insensitively into a small set of supported modes before dependency injection is configured. This prevents a harmless casing difference from silently routing SMS configuration to email, while unknown values fall back to the safer local email path. | `Services/NotificationProviderSelection.cs`, `Program.cs`, `NotificationProviderSelectionTests.cs` |
| Protected submission references | Data Protection and indirect object references | A public confirmation link should not expose a predictable database ID. The teleconsultation workflow protects the ID with ASP.NET Core Data Protection, verifies it before lookup, and separately limits patient history to the signed-in account or its verified email. | `Controllers/TeleconsultationsController.cs`, `Areas/Patient/Controllers/TeleconsultationsController.cs`, `TeleconsultationWorkflowTests.cs` |
| Progressive enhancement for booking | One workflow, multiple client capabilities | The JavaScript widget provides a faster interactive experience, but the ordinary HTML form remains a complete server-side path for slow connections or disabled scripts. Both paths enforce the same booking window, reserve the slot, notify the patient and staff, and publish realtime updates. | `Controllers/AppointmentRequestsController.cs`, `AppointmentWorkflowTests.cs` |
| Contact inbox workflow | Public validation and operational follow-through | A contact form is useful only when valid messages persist and staff can retrieve them. The server trims and validates untrusted input, SQL Server stores it, and the admin inbox orders, reads, and deletes the resulting record. | `Controllers/HomeController.cs`, `Areas/Admin/Controllers/ContactSubmissionsController.cs`, `ContactWorkflowTests.cs` |
| Public search publication boundary | Query scoping and content visibility | Search applies the selected content scope before querying and only exposes news marked as published. A real SQL Server test proves unpublished drafts stay out of public results while approved doctors, services, news, and patient guidance remain discoverable. | `Controllers/HomeController.cs`, `ContactWorkflowTests.cs` |
