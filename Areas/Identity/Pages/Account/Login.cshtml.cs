...
namespace Okafor_.NET.Areas.Identity.Pages.Account
{
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                // Count login failures towards account lockout (Lockout:MaxFailedAccessAttempts in config).
                var result = await _signInManager.PasswordSignInAsync(Input.Email, Input.Password, Input.RememberMe, lockoutOnFailure: true);
                if (result.Succeeded)
                {
                    _logger.LogInformation("User logged in.");

                    // If no specific return URL was requested, send admins/staff straight to the admin portal
